...
export class TreeSelect implements AfterContentInit {
  ...
  onSelectionChange(event: any) {
    ...
    let value =
      this.selectionMode === 'single'
        ? this.value.key
        : this.value.map((node: any) => node.key);
  ...
  updateTreeState() {
    if (
      (this.value && this.options && typeof this.value === 'string') ||
      (Array.isArray(this.value) && typeof this.value[0] === 'string')
    ) {
      // Map to nodes
      this.value = Array.isArray(this.value)
        ? this.value.map((key: any) => this.findByKey(key))
        : this.findByKey(this.value);
    }
    if (this.value) {
      let selectedNodes =
  ...
  findByKey(key: any): any {
    if (this.options) {
      for (let node of this.options) {
        let match = this.findNodeByKey(node, key);
  ...
  findNodeByKey(node: any, key: any): any | undefined {
    if (node.key === key) return node;
    for (let child of node.children) {
      let match = this.findNodeByKey(child, key);
      if (match) return match;
    }
    return undefined;
  }
